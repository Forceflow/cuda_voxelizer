...
thrust::host_vector<float3>* trianglethrust_host;
thrust::device_vector<float3>* trianglethrust_device;
...
float* meshToGPU_thrust(const trimesh::TriMesh *mesh) {
	...
	trianglethrust_host = new thrust::host_vector<float3>;
	trianglethrust_device = new thrust::device_vector<float3>;
	// fill host vector
	fprintf(stdout, "[Mesh] Copying %zu triangles to Thrust host vector \n", mesh->faces.size());
	for (size_t i = 0; i < mesh->faces.size(); i++) {
		float3 v0 = trimesh_to_float3(mesh->vertices[mesh->faces[i][0]]);
		float3 v1 = trimesh_to_float3(mesh->vertices[mesh->faces[i][1]]);
		float3 v2 = trimesh_to_float3(mesh->vertices[mesh->faces[i][2]]);
